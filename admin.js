...
function renderEventCard(e, count) {
  ...
  if (e.event_date) {
    ...
    if (today < start) { status = 'Before Event'; statusColor = 'var(--black)'; statusBg = '#f0f0f0'; }
    else if (today <= end) { status = 'Live'; statusColor = 'white'; statusBg = 'var(--red)'; }
    else { status = 'Ended'; statusColor = 'white'; statusBg = 'var(--black)'; }
  }
  const dateStr = e.event_date ? new Date(e.event_date).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' }) : 'No date set';
  const daysLabel = e.days > 1 ? e.days + ' days' : '1 day';
  const cardId = 'card-' + e.id;
    ...
    <div class="event-card" id="${cardId}">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
        <div>
          ...
          <p style="font-size:12px;color:var(--text-muted);margin-top:2px">${dateStr} · ${daysLabel}${e.event_code ? ' · <span style="font-family:monospace;background:var(--yellow);padding:1px 6px;border-radius:3px;color:var(--black);font-weight:700">' + esc(e.event_code) + '</span>' : ''}</p>
          ${e.mel_question ? `<p style="font-size:11px;color:var(--text-muted);margin-top:4px;font-style:italic">${esc(e.mel_question)}</p>` : ''}
        </div>
        <div style="text-align:right;flex-shrink:0;margin-left:1rem">
          ...
          <p class="count-num" style="margin-top:4px">${count}</p>
      ...
      <div style="margin:14px 0 10px">
        <button class="btn-action-red" style="width:100%;padding:12px;font-size:15px;font-weight:800;letter-spacing:0.02em" onclick="viewParticipants('${e.id}','${esc(e.name)}')">View Participants →</button>
      </div>
      ...
      <!-- Row 4: Management -->
      <div style="display:flex;justify-content:space-between;align-items:center;border-top:0.5px solid var(--border);padding-top:10px">
        <div style="display:flex;gap:6px;flex-wrap:wrap">
